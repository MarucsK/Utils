...
struct optional {
  ...
  public:
    ...
    explicit optional(
        InPlace, std::initializer_list<U> ilist, Ts &&...value_args)
    ...
    optional &operator=(const optional &other) {
        ...
        if ( other._has_value ) { new (&_value) T(other._value); }
    ...
    bool operator==(Nullopt) const noexcept { return !_has_value; }
    ...
    const T &value() const & {
        if ( !_has_value ) { throw BadOptionalAccess(); }
        return _value;
    }
    ...
    bool operator==(const optional<T> &other) const noexcept {
        ...
        if ( _has_value ) { return _value == other._value; }
        return true;
    }

    bool operator!=(const optional<T> &other) const noexcept {
        if ( _has_value != other._has_value ) { return true; }
        if ( _has_value ) { return _value != other._value; }
        return false;
    }
    ...
    template <
        class F,
        typename std::enable_if<std::is_copy_constructible<T>::value, int>::
            type = 0>
    ...
    template <
        class F,
        typename std::enable_if<std::is_move_constructible<T>::value, int>::
            type = 0>
