...
struct Function {
    static_assert(
        !std::is_same_v<_FnSig, _FnSig>, "not a valid function signature");
};
...
struct Function<_Ret(_Args...)> {
  ...
  public:
    ...
    template <
        ...
        class = std::enable_if_t<
            std::is_invocable_r_v<
                _Ret,
                std::decay_t<_Fn>,
                _Args...> && std::is_copy_constructible_v<_Fn> && !std::is_same_v<std::decay_t<_Fn>, Function<_Ret(_Args...)>>>>
    ...
    _Ret operator()(_Args... __args) const {
        if ( !_M_base ) [[unlikely]] { throw std::bad_function_call(); }
    ...
    _Fn *target() const noexcept {
        return _M_base && typeid(_Fn) == _M_base->_M_type() ? std::addressof(
