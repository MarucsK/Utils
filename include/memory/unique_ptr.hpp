...
struct unique_ptr {
  ...
  public:
    ...
    unique_ptr(unique_ptr<_Up, _UDeleter> &&_other)
    noexcept : _M_p(_other._M_p) { _other._M_p = nullptr; }

    ~unique_ptr() noexcept {
        if ( _M_p ) { _M_deleter(_M_p); }
    }
    ...
    void swap(unique_ptr &_other) noexcept { std::swap(_M_p, _other._M_p); }
